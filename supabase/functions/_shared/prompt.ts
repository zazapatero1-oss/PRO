...
export function buildIngestMergePrompt(
  ...
  const system = [
    ...
    "1. facets: for constructs already in the map, every distinct detail the instrument probes that the map's facets do not yet cover, as {id, label}: specific sub-features (e.g. for eyes: eyelids, eyelashes, eye colour match, how open they look), situations (photos, video calls, close up, from the side), reactions from others, and effects on daily life. Short ids in snake_case, labels in your own words. Be generous: it is normal to add 3–8 facets to a construct the instrument has a whole scale about. A construct may end up with up to 12 facets.",
    "2. new_constructs: whenever the instrument has a scale (or several items) about a feature, function or concern that no existing construct clearly covers — for example a facial feature that is missing from the map (forehead, head shape, jaws, nostrils, birthmark, ears), a function (eye function, hearing, using a hearing aid), scars or adverse effects tied to a specific site, or a distress theme (about speech, about being looked at). Full construct objects including 5–8 facets; each names the domain it belongs to, reusing an existing domain id where one fits.",
