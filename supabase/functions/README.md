...
Normal turn: `evidence*` → `token*` → `status`. Session end: `token*` → `status` → `ended`. Safety
halt: `safety` only (keyword) or `token*` → `safety` (model-raised). Pause: fixed `token` →
`status`. Stop: fixed `token` → `ended{patient_requested}`. After `ended` the client calls
`end-session`. Retrying a failed turn with the same `text` does not duplicate the patient message.
...
- `_shared/prompt.ts` — §7.2 system prompt (sections 1–8), §7.3 tool schemas, §8 and §11 prompts.
  Header comment carries the prompt file version.
